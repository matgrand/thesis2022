...
\begin{enumerate}
    \item \textbf{Data collection}: this step consists in collecting, in the simulated environment, the raw data that will be used to
...
\begin{itemize}
    ...
    on the path and then choosing the point on the path at a fixed distance ahead of it. This way of selecting
    the point ahead is very convenient and easy to implement and has the advantage of always finding
    a unique point, but is not conformed to the common definition of point ahead for the PP
    controller. 
...
The new data collection method is design with 3 main objectives: simplifying the 
...
%         -(camera position, requires redoing all videos)
    ...
    % talk about gazeebo, ros, the initial setup with automobile_data, then talk about the new setup,
    % that uses services to directly cotntrol the car and why it is more useful, than talk about the
    % path semplification, to eprove points not to be accurate

\subsection{Heading error calculation}
    Calculation of the heading error

%talk about the initial method that was fixed wrt the dataset, and then the new setup that allows
%for more flxibility, then talk about the algorithm, add image to explain algorithm

\subsection{Dataset}
    Dataset generation

%talk about the dataset generation pipeline, from the raw images to the preprocessing to the data augmentation


        
\subsection{Neural network training}
    Talk aboout the training
