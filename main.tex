...
\section{Introduction}
    ...
    addresses the problem of lane detection and tracking. The methods and the development are
    tailored for the specific application of the Bosch Future Mobility Challenge (BFMC), an
    internation autonomous driving competition for 1:10 scaled vehicles, in which we competed and
    won as the DEI-University of Padova team. 

    The main objective of this thesis is to develop a local estimation system, capable of running on
    an embedded device (a Raspberry Pi 4), that, together with an appropriate controller, is capable
    of keeping the autonomous vehicle in the lane, and tracking the center line. 
    ...
    metrics used to evaluate the estimation accuracy. 

    We will then propose a semi-original estimation method, which is based on the use of a neural
    network trained exclusively in the virtual environment. The network is fed with images from a
    camera mounted on the vehicle and outputs an estimate of the heading error of the vehicle w.r.t.
    the centerline, which is the estimation target. 
    ...
    application in a real-world scenario to evaluate the tracking performance. 
...
\subsection{Control loop application}
    ...
    \begin{figure}
        \centering
        \makebox[\textwidth][c]{\includegraphics[width=1.1\linewidth]{figures/full_control_loop.eps}}
